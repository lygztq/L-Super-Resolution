...
def model(input_tensor):
	with tf.device("/gpu:0"):
		...
		conv_w = tf.get_variable("conv_18_w", [3,3,64,1], initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0/9/64)))
		conv_b = tf.get_variable("conv_18_b", [1], initializer=tf.constant_initializer(0))
		weights.append(conv_w)
		weights.append(conv_b)
